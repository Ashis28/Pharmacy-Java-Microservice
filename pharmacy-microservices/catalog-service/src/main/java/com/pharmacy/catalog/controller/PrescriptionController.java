...
public class PrescriptionController {
    ...
    // Admin endpoints (also exposed as internal for admin-service)
    @GetMapping("/pending")
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<PrescriptionResponse> approve(@PathVariable Long id) {
        return ResponseEntity.ok(prescriptionService.approve(id));
    }

    @PutMapping("/{id}/reject")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<PrescriptionResponse> reject(@PathVariable Long id,
                                                        @RequestBody Map<String, String> body) {
